...
namespace RentHouse.Areas.Admins.Controllers
{
    ...
    public class PaymentController : Controller
    {
        ...
        {
            RoomHouse house = _db.roomHouses.FirstOrDefault(x=>x.Id==1);
            ...
            {
                Name = house.RoomNumber.ToString(),
                Currency = "USD",
                Price = house.PriceRent.ToString(CultureInfo.InvariantCulture),
                Quantity = 1.ToString(),
                Sku = "sku",
                Tax = "0"
            });
            ...
            var payment = new Payment()
            {
                ...
                Transactions = new List<PayPal.v1.Payments.Transaction>()
                {
                    new PayPal.v1.Payments.Transaction()
                    {
                        Amount = new Amount()
                        {
                            Total = house.PriceRent.ToString(CultureInfo.InvariantCulture),
                            Currency = "USD"
                        },
                        ItemList = itemList,
